...
public class Solver {
    ...
    private void solve(){
    	while(true){
        	...
        	if (searchNode.board.isGoal()){
        		...
        		break;
                    ...
                    loop:for (Board neiborBoard: searchNode.board.neighbors()){
        			SearchNode neiborNode = new SearchNode(neiborBoard, searchNode.move+1, searchNode, searchNode.isTwin);
        			boolean flag = false;       //indicate whether neiborBoard is the same as one of searchNode's board's ancesctor;
                                SearchNode now = searchNode;
                                while (now != null)
                                {
                                    if (neiborBoard.equals(now.board)) continue loop;
                                    now = now.parent;
                                }
        ...
        while (solutionQueue.size() > 0)
            solution.push(solutionQueue.pop());
    }
